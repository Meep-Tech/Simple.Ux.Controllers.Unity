...
namespace Simple.Ux.Controllers.Unity {
  ...
  public abstract class FieldController : MonoBehaviour, IFieldController {
    ...
    void Update() {
      if(isActiveAndEnabled && _dirtyForRow) {
    ...
    /// </summary>
    protected internal virtual void SwitchToRowMode(RowController forRow) {
      if(TryGetComponent<ContentSizeFitter>(out var contentSizeFitter)) {
        contentSizeFitter.horizontalFit = ContentSizeFitter.FitMode.PreferredSize;
      }
      if(TryGetComponent<VerticalLayoutGroup>(out var verticalGroup)) {
        Destroy(verticalGroup);
        _dirtyForRow = true;
      }
    }
    ...
    void _initializeTooltip() {
      if(!string.IsNullOrWhiteSpace(FieldData.Tooltip)) {
        TooltipController tooltip = TitleObject.AddComponent<TooltipController>();
